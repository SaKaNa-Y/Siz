...
- [01 — Is dependency governance being absorbed by the package managers?](tickets/01-governance-absorption.md) — the hypothesis **survives but narrows**: publish age is absorbed ecosystem-wide (pnpm's `minimumReleaseAge` is on by default since v11; npm, yarn and bun all shipped equivalents) and provenance nearly so, so positioning may lead with neither. What no one enforces: **install size** (total whitespace, free or Enterprise), **staleness as an admission rule** (every tool's age logic delays *new* things, never forbids *old* ones), and **one committable file spanning the families** — today that means five-plus tools across five config surfaces. Renovate and Dependabot are structurally incapable of admission control; every tool that gates does *audit-what's-there* or *gate-what's-coming*, never both. Yarn constraints is the right shape with no access to registry facts. The competitor the map missed: **`npq`** already covers 4/6 families and audits standalone — but its config is environment variables only.
...
- [05 — What can a policy actually say?](tickets/05-predicate-vocabulary.md) — **six rules** (`name`, `license`, `installSize`, `stale`, `deprecated`, `provenance`), each taking `severity` / `scope` / `ignore` and nothing else — flat, no composition, ever. The word **predicate** does not survive: `siz.config.json` is the *policy*, each entry is a *rule*, and `rules` extends in place, so today's `allow`/`deny` become a permanent, never-deprecated alias for the `name` rule. The absorption rule that generalises: **siz defines a rule when the question is "what is in this project," and delegates when it is "may this version be adopted right now"** — cooldown goes to the package manager, staleness stays siz's, provenance is kept anyway to keep the one-file claim whole. Popularity is never policy; bundle size fails cold CI. Severity defaults to `error`; unknown facts never violate but escalate via an audit-only flag; anything siz cannot fully understand aborts; merge semantics are fixed per key **now** (scalars replace, deny/ignore union, allow intersects), and `extends` is a convenience, not a control.
...
- **A vulnerability rule.** Whether advisory data belongs in the policy at all, given it means a new data source and a new trust relationship. (Would be a seventh rule under 05's vocabulary; its inclusion criterion is the test to apply.)
...
- **SPDX expression evaluation.** Whether compound license expressions are ever really evaluated rather than reported as unevaluated. Ticket 05 deliberately left this open: `license.allow`/`deny` match the declared string with the same glob engine `name` uses, so `(MIT OR GPL-3.0)` is matched as text today.
