...
- [x] Weekly download counts inline on every result, in place of npm's retired score bars (the search endpoint now returns a constant `1.000` for quality, popularity and maintenance on every package); scoped packages included via npm's single-package endpoint
...
- [x] Deprecation and provenance read from the registry packument siz already fetches per result, leaving the third-party metadata service responsible for publish age alone
- [x] Signal fetches bounded to the rows on screen (plus a prefetch margin) — fewer requests per search, unchanged progressive rendering; `--list` / `--json` still cover every result they print
...
| `✓`   | **Provenance** — the published version carries a build attestation   |

Alongside the glyphs, every row leads with the package's **weekly download count** — `250.1M/wk`, `1.5k/wk` — so adoption is comparable at a glance. It replaces npm's old `quality ▰▰▰▰▰  popularity ▰▰▰▰▰` bars, which the registry retired: the search endpoint now returns a constant `1.000` for quality, popularity and maintenance on *every* package, so those bars were structurally always full. Unscoped packages get the count from the same download data the `↑`/`↓` momentum arrow already needs, at no extra cost; scoped packages (`@types/node`, `@tanstack/react-query`) — which npm's bulk endpoint rejects — get a last-week count from its single-package endpoint instead, which is why they show a count but never an arrow. A count siz couldn't fetch renders nothing rather than a zero. See [ADR 0011](./docs/adr/0011-weekly-downloads-replace-score-bars.md).

The glyphs show on every row so you can compare at a glance; the focused row expands them to words (e.g. `250.1M weekly downloads · deprecated: no longer maintained · published 4y ago`). When a deprecation message names a successor, that focused detail (and `--list`/`--json`) also surfaces it as `→ replaced by <pkg>` — parsed straight from the message, so it reflects what the maintainer pointed to, not a recommendation siz invents (a deprecated package whose message names no successor simply shows none). `--json` adds a `replacedBy` array per result. Signals are purely informational — they never block, filter, or reorder results. They load progressively (the list never waits on them) and degrade silently — independently — if their source is unreachable: the npm packument for deprecation and provenance (the same request that already fetches install size and the license), the metadata service (`fast-npm-meta`) for publish age alone, npm's download API for the count and the arrow (see [Data sources & network](#data-sources--network)). The `--list` and `--json` outputs include them too (`--json` adds `deprecated`, `publishedAt`, `provenance`, and `downloads` fields per result — `downloads` absent, never `0`, when the count is unknown). `--json` carries **no** `score` or `searchScore` field: the three score numbers are constants now, and the registry's relevance number survives only inside siz as the last tiebreaker in name-affinity ranking. Deprecation and provenance come from the registry's own packument rather than a third-party aggregator, which is also why `✓` means specifically "the published version carries a provenance attestation" — see [ADR 0012](./docs/adr/0012-packument-sourced-deprecation-provenance.md).
...
Drop a committable `siz.config.json` at your repo root to declare which packages may be installed. Siz reads it and **blocks disallowed packages at install time** — at all three install paths: the interactive **Install** action, direct `siz add`, and `siz bundle install`.
...
Bundles are the only place packages are saved: the front door (Enter on an empty search box) and `siz list` both show saved bundle entries. Favorites were removed — if you had any, they were migrated into a bundle named `favorites` on first run, so `siz list -b favorites` shows them. `siz add --fav` and `siz rm --fav` now error with the flow that replaced them: save with `siz add <pkg> --bundle <name>`, remove with `siz bundle rm <name> <pkg>`. See [ADR 0010](./docs/adr/0010-bundles-as-the-only-saved-package-store.md) for why there is one store rather than two.
...
| ------- | -------- | -------- |
| Package search (interactive, `--list`, `--json`) | `registry.npmjs.org/-/v1/search` | Official npm registry |
| Publish age (trust signal, `⚑` stale) | `npm.antfu.dev` (via [`fast-npm-meta`](https://github.com/antfu/fast-npm-meta)) | Third-party hosted aggregator |
| Deprecation + provenance (trust signals, `⚠` / `✓`) | `registry.npmjs.org/<pkg>` (packument — shared with install size and license, no extra request) | Official npm registry |
...
| Weekly download count + trend momentum (`↑`/`↓`) | `api.npmjs.org/downloads` (bulk point endpoint; the single-package one for scoped names) | Official npm download-counts API |
...
Publish age, upgrades, and bundle resolution go through **`fast-npm-meta`**, whose default API endpoint is **`https://npm.antfu.dev/`** — a third-party service (maintained by [antfu](https://github.com/antfu)) that mirrors and aggregates the npm registry so this data can be fetched in a single batched request. These calls **degrade silently** if the service is unreachable (the `⚑` stale flag simply doesn't appear; upgrades/bundles surface the failure). Note that `fast-npm-meta` cannot be pointed at the raw `registry.npmjs.org` — it speaks its own aggregation protocol — so removing this dependency would require self-hosting that API or reimplementing the fetches. Everything else comes from **official npm endpoints** directly: package search, **weekly download counts** and trend momentum, and the four facts that ride one packument request per package — **install size** (`dist.unpackedSize`), the **license signal** (`license`), **deprecation** (`deprecated`) and **provenance** (`dist.attestations`). Deriving the last two from the packument means siz reads them from the registry it is installing from, rather than buying them a second time from an aggregator, and it costs no extra network at all. **Bundle size** is the one signal from another third party, [Bundlephobia](https://bundlephobia.com) — fetched only for the focused search row and degrading silently like the rest. In the interactive box these per-result fetches are bounded to the rows on screen plus a small prefetch margin, with the rest filling in as you scroll (nothing is fetched twice); `--list` and `--json` print every result, so they fetch signals for every result. See [ADR 0003](./docs/adr/0003-fast-npm-meta-hosted-endpoint.md), [ADR 0008](./docs/adr/0008-package-size-data-sources.md), [ADR 0009](./docs/adr/0009-license-signal-data-source.md), [ADR 0011](./docs/adr/0011-weekly-downloads-replace-score-bars.md), and [ADR 0012](./docs/adr/0012-packument-sourced-deprecation-provenance.md) for the rationale.
