...
Commands:
  ...
  upgrade [level]                   Upgrade project dependencies (level: major | minor | patch)
...
Options:
  ...
  --no-rules      Bypass dependency rules in siz.config.json when installing
...
Per-command help lists each command's own options: `siz add --help` shows `-b/--bundle` (its one mode flag), `-D/--dev`, `-s/--strategy`, `--no-rules`; `siz rm --help` has no mode flag and no options; `siz list --help` shows `-b/--bundle`; `siz bundle --help` shows `--no-rules` (for `bundle install`); `siz upgrade --help` shows `-r/--recursive` and `--dry-run`; `siz outdated --help` shows `-r/--recursive`, `--json`, and `--exit-code`.
...
- `upgrade [level]` / `up` (alias) → `commands/upgrade.ts`; `level` is `major | minor | patch` (validated by `core/upgrade.parseUpgradeMode()`, defaults to `major` = newest overall); `-r/--recursive` discovers every `package.json` under cwd (monorepo mode); `--dry-run` previews without writing or installing
...
- `upgrade.ts` — the upgrade specialization of `compare.ts`: `analyzeDep()`/`resolveTarget()` pick a target under a mode ceiling (`UpgradeMode`, `parseUpgradeMode()`), `buildUpgradePlan()`/`planManifests()` extend that across multiple manifests (`ManifestPlan`), `planCatalog()` does the same for pnpm catalog entries (`CatalogPlanItem`); types `DepAnalysis`, `UpgradePlan`
- `outdated.ts` — read-only analysis for `siz outdated`: `analyzeOutdated()` classifies one dep as outdated / skipped / up-to-date (Current = range floor, Wanted = `maxSatisfying` over the literal range, Latest = dist-tag); `buildOutdatedReport()`, `planManifestsOutdated()` (`ManifestOutdated`), and `planCatalogOutdated()` (`CatalogOutdatedItem`) span manifests + catalog. Reuses `compare.ts`'s `compareDep`/`currentVersionFromRange`/`safeDiff`; reports complex ranges (unlike `upgrade.ts`, which must skip them to rewrite safely). Never writes
...
- `bundle.ts` — resolve a bundle against the registry (one batched request) into an install plan: `resolveBundleInstall()` applies each entry's version strategy to a **freshly** resolved latest (never snapshotted); `isPinned()` narrows an exact entry
- `compare.ts` — the neutral registry-comparison core shared by upgrade and outdated (ADR 0007): `compareDep()` plus the pure `detectRangePrefix`/`applyPrefix`/`currentVersionFromRange`/`safeDiff`/`stableCandidates`; types `CompareResult`/`DepComparison`, `VersionInfo`, `DiffLevel`, `RangePrefix`, `CompareSkip`. It computes only the shared facts — each command specializes them — and owns the one batched registry call both make, `fetchVersionInfo()` (`fast-npm-meta`)
- `resolve.ts` — the shared discovery front-half of `upgrade`/`outdated` (and the planned `siz check`): `discoverProjectDeps()` returns a `DependencyScan` (manifests + nearest catalog + the deduped upgradable names from `collectQueryNames()`/`collectCatalogNames()`). Discovery only — it fetches nothing
- `text.ts` — `escapeRegExp()`, shared by the manifest/catalog rewriters, the rules globber and `ui/highlight.ts`
...
**UI** (`src/ui/`): `render.ts` (result cards, `formatBlockedNotice` for rule-blocked packages; `downloadsInline`, the weekly count every row leads with (it replaced npm's retired score bars); one `*Inline`/`*Detail` formatter pair per result-signal family — `trustGlyphs`/`trustDetail`, `sizeInline`/`sizeDetail`, `licenseInline`/`licenseDetail` — and `signalLegend()` covering all three families' glyphs), `bundle-render.ts` (saved-entry and bundle list/show lines — `renderSavedEntryLine()` backs both `siz list` and the front door — plus `formatBundleRemoval()` and `bundleInstallOptionLabel()`), `search-prompt.ts` (type-as-you-search multiselect), `prompts.ts` (`@clack/prompts` confirm/action menus, package-manager picker), `upgrade-render.ts` (upgrade summary line, version deltas, option labels; exports `colorForDiff`), `outdated-render.ts` (aligned Current/Wanted/Latest table + summary line, reusing `colorForDiff`), `highlight.ts` (keyword highlighting). Color via `ansis`.

**Library surface** — `src/index.ts` re-exports core functions (`searchPackages`, `detectPM`, `buildInstallCommand`, `buildRemoveCommand`, `parseSpec`, `listSavedEntries`, `core/bundle.ts`'s install-plan resolution, the `project.ts`/`resolve.ts`/`compare.ts`/`upgrade.ts`/`catalog.ts` scan-compare-upgrade functions and types, the `outdated.ts` report functions and types, the `size.ts`/`license.ts`/`packument.ts` result-signal functions, …). Keep it in sync when core signatures change.
